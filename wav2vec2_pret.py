...
from old_config import Wav2Vec2Config
#import datasets
import numpy as np
...
from transformers.models.wav2vec2.modeling_wav2vec2 import _compute_mask_indices, _sample_negative_indices, Wav2Vec2PreTrainedModel, Wav2Vec2Model
...
def parse_args():
    ...
    # Dataset and model
    parser.add_argument("--dataset_config_names", type=str, nargs="+", default=["default", "default"])
    parser.add_argument("--dataset_split_names", type=str, nargs="+", default=["train.clean.100"])
    parser.add_argument("--model_name_or_path", type=str, default="patrickvonplaten/wav2vec2-base-v2")
    parser.add_argument("--pretrained_output_dir", type=str, required=True)
    ...
    parser.add_argument("--num_warmup_steps", type=int, default=32000)
    ...
    parser.add_argument("--audio_column_name", type=str, default="eeg")
    parser.add_argument("--validation_split_percentage", type=int, default=1)
    ...
    parser.add_argument("--preprocessing_only", type=bool, default=None)
    parser.add_argument("--max_gumbel_temperature", type=float, default=2)
...
train_idx, val_idx = train_test_split(np.arange(len(trainy)), test_size=0.20, random_state=42, shuffle=True, stratify=trainy_label)
...
args.max_train_steps = 100 * num_update_steps_per_epoch
...
logger.info(f"  Num examples = {len(eeg_dataset['train'])}")
...
#             if (args.push_to_hub and epoch < args.num_train_epochs - 1) or args.pretrained_output_dir is not None:
#                 accelerator.wait_for_everyone()
#                 unwrapped_model = accelerator.unwrap_model(model)
#                 unwrapped_model.save_pretrained(
#                     args.pretrained_output_dir, is_main_process=accelerator.is_main_process, save_function=accelerator.save
#                 )
...
# unwrapped_model = accelerator.unwrap_model(model)
# unwrapped_model.save_pretrained(
#     args.pretrained_output_dir,
#     is_main_process=accelerator.is_main_process,
#     save_function=accelerator.save
# )
#model = Wav2Vec2PreTrainedModel.from_pretrained(args.pretrained_output_dir)
model = Wav2Vec2ForSequenceClassification.from_pretrained(args.pretrained_output_dir, num_labels=5, problem_type ="multi_label_classification")
config = model.config
optimizer = AdamW(
    list(model.parameters()),
    lr=args.learning_rate,
...
for epoch in range(starting_epoch, 100):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        batch.pop('sampled_negative_indices', None)
